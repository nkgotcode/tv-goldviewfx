...
test.skip(!process.env.E2E_RUN, "Set E2E_RUN=1 to enable Telegram dedup e2e tests.");

test("Telegram ingestion deduplicates messages", async () => {
  ...
  const hasDuplicate = posts.some((post: { dedup_status: string }) => post.dedup_status === "duplicate");
