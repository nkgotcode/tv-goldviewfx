...
if (!hasEnv) {
  test.skip("quality metrics require Supabase configuration", () => {});
} else {
  test("sync runs store quality metrics", async () => {
    ...
    const result = await supabase.from("sync_runs").select("coverage_pct, missing_fields_count, parse_confidence").eq("id", run.id).single();
