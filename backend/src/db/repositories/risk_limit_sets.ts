...
export async function insertRiskLimitSet(payload: RiskLimitSetInsert) {
  const result = await supabase.from("risk_limit_sets").insert(payload).select("*").single();
...
export async function updateRiskLimitSet(id: string, payload: Partial<RiskLimitSetInsert>) {
  const result = await supabase.from("risk_limit_sets").update(payload).eq("id", id).select("*").single();
  return assertNoError(result, "update risk limit set");
}

export async function listRiskLimitSets(filters: { activeOnly?: boolean } = {}) {
  const query = supabase.from("risk_limit_sets").select("*").order("effective_from", { ascending: false });
...
export async function getRiskLimitSet(id: string) {
  const result = await supabase.from("risk_limit_sets").select("*").eq("id", id).single();
